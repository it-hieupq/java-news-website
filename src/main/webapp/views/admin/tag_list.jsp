  ...
  <!-- MAIN CONTENT -->
      <div>
        ...
        <div class="container">
          ...
          <h3 align="center"><a href="<c:url value='/admin/tags'/>?action=create" class="p-10"> New tag</a></h3>
	          ...
	          <div class="pagination mt-2 mb-2" align="center">
	          	<c:forEach begin="1" end="${totalPage }" varStatus="num">
	          		<c:if test="${num.index!=currentPage }">
			          	<a href="${pageContext.request.contextPath}/admin/tags?action=list&page=${num.index}"> 
				          	<button class="button-paginate"> ${num.index } </button> 
			          	</a>
		          	</c:if>
		          	
		          	<c:if test="${num.index==currentPage }"> 
		          		<button class="button-paginate" style="background-color: red;"> ${num.index } </button>
		          	</c:if>
